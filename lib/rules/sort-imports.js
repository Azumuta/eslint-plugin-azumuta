...
import importTypePlugin from 'eslint-plugin-import/lib/core/importType.js';

export const SORT_IMPORTS_ERROR_MESSAGE = 'Imports not sorted correctly';
...
const DEFAULT_IMPORT_TYPE_ORDER = ["builtin", "external", "internal", "parent", "sibling", "index"];
...
function buildList(list, context) {
    let catchAll;
    ...
    return list
        .reduce((list, arr) => {
            ...
            if (!string) {
                ...
                if (string === '*') {
                    catchAll = (importName) => {
                        const importType = importTypePlugin.default(importName, context);
                        // adding order of importType, this way the correct order will be used
                        return (order + 0.1 * (DEFAULT_IMPORT_TYPE_ORDER.indexOf(importType) ?? 9));
                    }
                    return list;
                }
...
export default {
    ...
    create: function (context) {
        const orderList = buildList((context.options.length && context.options[0]) || DEFAULT_ORDER_LIST, context);
        let imports = [];
        let done = false;
        ...
        function importsDone() {
            if (imports.length > 1) {
                ...
                if (!imports.every((decl, i) => decl === ordered[i])) {
                    context.report({
                        node: imports[0],
                        message: SORT_IMPORTS_ERROR_MESSAGE,
