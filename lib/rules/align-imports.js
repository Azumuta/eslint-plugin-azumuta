...
export const ALIGN_IMPORTS_MESSAGE = 'Imports not aligned correctly';
...
function generateImportText(currentImport, defaultSpecifierText, importNamespaceSpecifierText,
    ...
    let newLine = 'import ';
    if (!defaultSpecifierText && !importNamespaceSpecifierText && specifierTexts.length === 0) {
        // special case import    'bsdfsdfsd';
        // subtract 7 characters for 'import '
        newLine += ' '.repeat(newSourceStartColumn - 7) + currentImport.source.raw + ';';
        return newLine;
    }
    if (currentImport.importKind && currentImport.importKind === 'type') {
        newLine += 'type ';
    }
    let shouldPrependComma;
    ...
    if (importNamespaceSpecifierText) {
        ...
        if (defaultSpecifierText) {
            // for ' from '
            if (newLine.length + defaultSpecifierText.length + 6 > newSourceStartColumn) {
                if (!isMultiLine) {
                    return false;
                }
    ...
    if (isMultiLine) {
        ...
        newLine += ' '.repeat(newSourceStartColumn - 5 - 1);
    } else {
    ...
    if (currentImport.attributes && currentImport.attributes.length) {
        newLine += ' assert { ';
        shouldPrependComma = false;
        for (const attribute of currentImport.attributes) {
            if (shouldPrependComma) {
                newLine += ', ';
            } else {
                shouldPrependComma = true;
            }
            newLine += attribute.key.name + ': ' + attribute.value.raw;
        }
        newLine += ' }';
    }
    newLine += ';';
