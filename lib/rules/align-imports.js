...
function generateImportText(currentImport, defaultSpecifierText, importNamespaceSpecifierText,
    ...
    if (currentImport.assertions && currentImport.assertions.length) {
        newLine += ' assert { ';
        shouldPrependComma = false;
        for (const assertion of currentImport.assertions) {
            if (shouldPrependComma) {
                newLine += ', ';
            } else {
                shouldPrependComma = true;
            }
            newLine += assertion.key.name + ': ' + assertion.value.raw;
        }
        newLine += ' }';
    }
    newLine += ';';
